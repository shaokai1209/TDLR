...
%     A=(Xs*Xs'+X*(lambda*Mc+alfa*L)*X'+beta*eye(size(X,1))+gamma*G)\(Xs*(Y+B.*M));%tempp*(Xs*(Y+B.*M))
      A=(u1*Xt*Xt'+Xs*Xs'+X*(lambda*Mc+alfa*L)*X'+beta*eye(size(X,1))+gamma*G)\(Xs*(Y+B.*M)+u1*Xt*Q*(Y+B.*M));
%     Y = Xs'*A\(eye(size(L))-L)-B.*M;
%     lo=pinv(u1*(Y+B.*M)*(Y+B.*M)'-u2*Z);
%     Q=(u1*Xt'*A*(Y+B.*M)')*lo;
    Q=(u1*Xt'*A*(Y+B.*M)')/(u1*(Y+B.*M)*(Y+B.*M)'-u2*Z);
    F=Xs'*A-Y;
    M_original=B.*F;
    [x11,y11]=size(F);
    %%
...
%     else
    ...
    sam3 = Xt'*A - Q*(Y+B.*M);
    sam4 = A'*X*Mc*X'*A;
    obj(iter)=trace(sam1'*sam1)+u1*trace(sam3'*sam3)+alfa*trace(sam2)+lambda*trace(sam4);
    if iter>2
